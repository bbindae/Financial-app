...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Extract the path after /api/yahoo-options/
  ...
  const url = `https://query2.finance.yahoo.com/v7/finance/options/${subPath}?${queryParams.toString()}`;
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      if (retryAuthed) {
        ...
        const retryUrl = `https://query2.finance.yahoo.com/v7/finance/options/${subPath}?${queryParams.toString()}`;
